...
# print(f"the minimun values is : {min(values)}")
# print(f"the minimun values is : {min(strings)}")

# # TODO: The max() function finds the maximum value

# print(f"the minimun values is : {max(values)}")
# print(f"the minimun values is : {max(strings)}")
# # TODO: define a custom "key" function to extract a data field
# print(f"the minimun values is : {min(strings, key=len)}")
# print(f"the minimun values is : {max(strings, key = len)}")

# TODO: open the data file and load the JSON
with open("/Users/ranadheerkarasani/Downloads/Code/data_analysis_with_python/30DayQuakes.json", "r") as datafile:
